...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if (substr($userprofile, 0, 1) == "*") {
    ...
    $userprofile = isset($getprofile[0]['.id']) ? $getprofile[0]['.id'] : "";
    if ($userprofile == "") {
  ...
  $profiledetalis = isset($getprofile[0]) ? $getprofile[0] : array();
  ...
  $ppool = tikras_array_get($profiledetalis, 'address-pool', '');
  $sparent = tikras_array_get($profiledetalis, 'parent-queue', '');

  if(empty($ppool)){$ppool = "none";}
  if(empty($sparent)){$sparent = "none";}

  $onLoginParts = explode(",", $ponlogin);
  $getexpmode = tikras_array_get($onLoginParts, 1, '');
  ...
  $getgracep = tikras_array_get($onLoginParts, 4, '');

  $getlocku = tikras_array_get($onLoginParts, 6, '');
  if ($getlocku == "") {
    $getlocku = "Disable";
  } else {
    $getlocku = $getlocku;
  ...
  if (tikras_has_post('name')) {
    $name = (preg_replace('/\s+/', '-', tikras_post('name')));
    $sharedusers = tikras_post('sharedusers');
    $ratelimit = tikras_post('ratelimit');
    $expmode = tikras_post('expmode');
    $validity = tikras_post('validity');
    $graceperiod = tikras_post('graceperiod');
    $getprice = tikras_post('price');
    $getsprice = tikras_post('sprice');
    $addrpool = tikras_post('ppool');
    if ($getprice == "") {
      $price = "0";
    } else {
      $price = $getprice;
    ...
    $getlock = tikras_post('lockunlock');
    if ($getlock == "Enable") {
...
//       "transparent-proxy" => "yes",
    ...
    tikras_redirect('./?user-profile=' . $pid . '&session=' . $session);
  }
}
?>
<div class="row">
