...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if (substr($hotspotuser, 0, 1) == "*") {
    ...
    $hotspotuser = isset($getuser[0]['.id']) ? $getuser[0]['.id'] : "";
  ...
  $userdetails = isset($getuser[0]) ? $getuser[0] : array();
  ...
  $profiledetalis = isset($getprofilebyuser[0]) ? $getprofilebyuser[0] : array();
  $ponlogin = tikras_array_get($profiledetalis, 'on-login', '');
  $onLoginParts = explode(",", $ponlogin);
  $getvalid = tikras_array_get($onLoginParts, 3, '');
  $getprice = tikras_array_get($onLoginParts, 2, '');
  $getsprice = tikras_array_get($onLoginParts, 4, '');


  $getsch = $API->comm("/system/scheduler/print", array(
    "?name" => "$uname",
  ));
  $schdetails = isset($getsch[0]) ? $getsch[0] : array();
  $start = tikras_array_get($schdetails, 'start-date', '') . " " . tikras_array_get($schdetails, 'start-time', '');
  $end = tikras_array_get($schdetails, 'next-run', '');
...
if (in_array($currency, $cekindo['indo'])) {
  ...
  if (tikras_has_post('name')) {
    ...
    $comment2 = tikras_post('comment2');
    $hcomment = tikras_post('h_comment');
    ...
    tikras_redirect('./?hotspot-user=' . $uid . '&session=' . $session);
  }
}
...
<form autocomplete="new-password" method="post" action="">
  <div>
    ...
    <?php $del_confirm = "if(confirm('Are you sure to delete username (" . tikras_js_string($uname) . ")?')){loadpage('./?remove-hotspot-user=" . rawurlencode($uid) . "&session=" . rawurlencode($session) . "')}else{}"; ?>
    <div class="btn bg-danger"  onclick="<?= tikras_h($del_confirm) ?>" title="Remove <?= tikras_h($uname) ?>"><i class='fa fa-minus-square'></i> <?= $_remove ?></div>
    <?php $printJS = "window.open('./voucher/print.php?user=" . tikras_js_string($usermode . "-" . $uname) . "&qr=no&session=" . tikras_js_string($session) . "','_blank','width=310,height=450').print();"; ?>
    <?php $printQRJS = "window.open('./voucher/print.php?user=" . tikras_js_string($usermode . "-" . $uname) . "&qr=yes&session=" . tikras_js_string($session) . "','_blank','width=310,height=450').print();"; ?>
    <a class="btn bg-secondary"  title="Print" href="javascript:<?= tikras_h($printJS) ?>"> <i class="fa fa-print"></i> <?= $_print ?></a>
    <a class="btn bg-info"  title="Print QR" href="javascript:<?= tikras_h($printQRJS) ?>"> <i class="fa fa-qrcode"></i> <?= $_print_qr ?></a>
    ...
    <a id="shareWA" class="btn bg-green" title="Share WhatsApp" href="<?= tikras_h('whatsapp://send?text=' . rawurlencode($shareWA)) ?>"> <i class="fa fa-whatsapp"></i> <?= $_share ?></a>
