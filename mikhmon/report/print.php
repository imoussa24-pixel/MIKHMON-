...
function mikhmon_print_report_day($date)
{
	$parts = explode('-', $date);
	if (count($parts) >= 3) {
		return (int) $parts[2];
...
if (!isset($_SESSION["mikhmon"])) {
  ...
  $API = tikras_routeros_create();
	...
	$idbl2 = (isset($idhrParts[0]) ? $idhrParts[0] : "") . (isset($idhrParts[2]) ? $idhrParts[2] : "");
	if ($idhr != ""){
	...
	$prefix = tikras_get('prefix');
	$fcomment = tikras_get('comment');
	$range = tikras_get('range');
	$trange = "";
	if(!empty($range)){$trange = "[".$range."]";}
	
	$pcomment = substr($prefix, 0,2);
	if($pcomment == "!!"){
		$commentParts = explode("!!", $prefix, 2);
    ...
    window.print();
		  ...
		  <div class="overflow box-bordered" style="max-height: 70vh">
			...
			if ($fcomment != "" || $pcomment == "!!") {
			  ...
			  $x = isset($rangeParts[0]) ? (int) $rangeParts[0] : 0;
			  $y = isset($rangeParts[1]) ? (int) $rangeParts[1] : $x;

        $range = range($x, $y);
        
				for ($i = 0; $i < $TotalReg; $i++) {
					$getname = explode("-|-", $getData[$i]['name']);
					$day = mikhmon_print_report_day($getname[0]);
