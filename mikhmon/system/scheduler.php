...
for ($i = 0; $i < $TotalReg; $i++) {
	...
	echo "<tr>";
	?>
  	<td style='text-align:center;'><i class='fa fa-minus-square text-danger pointer' onclick="if(confirm('Are you sure to delete scheduler (<?= tikras_h(tikras_js_string($name)); ?>)?')){loadpage('./?remove-scheduler=<?= tikras_h(rawurlencode($id)); ?>&session=<?= tikras_h(rawurlencode($session)); ?>')}else{}" title='Remove <?= tikras_h($name); ?>'></i>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
  	<?php

		if ($disabled == "true") {
			$uriprocess = "'./?enable-scheduler=" . rawurlencode($id) . "&session=" . rawurlencode($session) . "'";
			echo "<span class='text-warning pointer' title='Enable Scheduler " . tikras_h($name) . "'  onclick=loadpage(" . tikras_h($uriprocess) . ")><i class='fa fa-lock '></i></span></td>";
		} else {
			$uriprocess = "'./?disable-scheduler=" . rawurlencode($id) . "&session=" . rawurlencode($session) . "'";
			echo "<span  class='pointer' title='Disable Scheduler " . tikras_h($name) . "'  onclick=loadpage(" . tikras_h($uriprocess) . ")><i class='fa fa-unlock '></i></span></td>";
		}
		echo "<td>" . tikras_h($name) . "</td>";
		echo "<td>" . tikras_h($startd) . "</td>";
		echo "<td>" . tikras_h($startt) . "</td>";
		echo "<td>" . tikras_h($interval) . "</td>";
		echo "<td>" . tikras_h($nextrun) . "</td>";
		echo "<td>" . tikras_h($runcount) . "</td>";
